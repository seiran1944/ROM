...
function Lua_703126_EvilAssimilation_EffectCheck()
	...
	--DebugMsg( 0 , 0 , "CheckTarget : ".._ownerID.."  ".._currentBuffLv );
	if ( _currentBuffLv >= 1 ) then--受到3個「穢邪同化」效果，就會化身成為棄靈選者
		
		--AddBuff( _ownerID , _corruptSpiritBuffID , 1 , -1 );--成為棄靈選者
		AddBuff( _ownerID , _corruptSpiritBuffID , 0 , -1 );--成為棄靈選者
		CancelBuff_NoEvent( _ownerID , _thisBuffID );--移除穢邪同化
	...
	DebugMsg( 0 , 0 , "目標有效 : ".._ownerID.." 目前 : "..Lua_703107_Return_Buff_Lv( _ownerID , _thisBuffID ) );
	return true;
end
...
function Lua_703126_EvilAssimilation_Infection()--625390穢邪同化	傳染機制
	...
	AddBuff( _ownerID , _thisBuffID , 0 , 60 );
	...
	for i = 0 , #_RangePlayerTable , 1 do--挑選傳染有效玩家
	...
	if ( #_EffectivePlayerTable <= 0 ) then
		DebugMsg( 0 , 0 , "There's no neighbor to infect: ".._ownerID );
		return;
	end
	...
	AddBuff( _currentPlayerID , _thisBuffID , 0 , 60 );--感染
...
function Lua_703126_CrushingSlay()
	...
	CallPlot( _dummyID , "Lua_703126_CrushingSlay_Execute" , 5 );

	return true;
end

function Lua_703126_CrushingSlay_Execute( )
