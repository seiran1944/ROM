...
local _Lua_703176_AI_GlobalVars = nil;

function Lua_703176_AI_Init( _inputCtrID )--controller  NPC 109085 --Note : this NPC is using as a ignitor
	DebugMsg( 0 , 0 , "--Lua_703176_AI_Init : NPC 109085       Z34  V 140528 " );
	...
	----------------------------END----------狂暴參數
	--請依難易度順序放置
	local _bossGuardGUIDs = { 109016 , 109016 , 109016 };--勾卡爾
	...
	--參數設定表
	if ( _Lua_703176_AI_GlobalVars == nil ) then _Lua_703176_AI_GlobalVars = {}; end
	_Lua_703176_AI_GlobalVars[ _roomID ] = {	--non 
					_bossID = 0 ,--以_Schulize做代表( 讓其他函式好找 _bossID)

					_bossVOs = {
						_Guard = {
							_id = 0 ,					--ID
							_guid = _bossGuardGUIDs[ _difficulty ] ,		--GUID
							_flagPos = 1 , 					--旗標位置
					...
					_flagGUID = 781424 , --781424 Z34_2王用旗標
	...
	--唯讀資訊
	local _gConsts = _Lua_703176_AI_GlobalVars[ _roomID ];

	--產生 Boss
	local _bossID = 0;
	for _bossName , _bossVO in pairs( _gConsts._bossVOs ) do
		_bossID = Lua_703176_GenerateBoss( _bossVO , _roomID );
		_Lua_703176_AI_GlobalVars[ _roomID ]._bossVOs[_bossName]._id = _bossID;--
	end
	_Lua_703176_AI_GlobalVars[ _roomID ]._bossID = _gConsts._bossVOs._Guard._id;--presiding Boss --Guard
	--Lua_703176_BossIndividualsSetting( _gConsts );

	--產生後門
	_Lua_703176_AI_GlobalVars[ _roomID ]._backDoorID = Lua_703182_AddDoor( _gConsts._doorGUID , _gConsts._flagGUID , _gConsts._backDoorFlagPos , _gConsts._roomID );
	DebugMsg( 0 , 0 , "--Back Door ID : ".._Lua_703176_AI_GlobalVars[ _roomID ]._backDoorID );

	--delete trigger
	DelObj( _ctrlID );
		
	return _roomID;
end--function Lua_703176_AI_Init( _inputCtrID )

function Lua_703176_GenerateBoss( _bossVO , _roomID )
	...
	return _bossID;
end--function Lua_703176_GenerateBoss( _bossVO )

function Lua_703176_ClearAll( _specRoomID )--for test
	if ( _Lua_703176_AI_GlobalVars ~= nil ) then
		DebugMsg( 0 , 0 , "--Lua_703176_ClearAll : all values will be cleared , reinit this by redeploying NPC 109085" );--
		...
		if ( _specRoomID ~= nil ) then
			local _valuePack = _Lua_703176_AI_GlobalVars[ _specRoomID ];
			if ( _valuePack ~= nil ) then
				ClearData( _valuePack );
			end
			_Lua_703176_AI_GlobalVars[ _specRoomID ] = nil;
			return;
		end

		for _roomID , _valuePack in pairs( _Lua_703176_AI_GlobalVars ) do
			ClearData( _valuePack );
			_Lua_703176_AI_GlobalVars[ _roomID ] = nil;
		end
	end
end--function Lua_703176_ClearAll()

function Lua_703176_AI_reignite( _specRoomID )--for test
	DebugMsg( 0 , 0 , "--Lua_703176_AI_reignite " );
	local _roomID = _specRoomID or ReadRoleValue( OwnerID() , EM_RoleValue_RoomID );
	Lua_703176_ClearAll( _roomID );
	Lua_703176_AI_Init();
end


function Lua_703176_AI_onFight()--當有多隻王時 只放在其中一隻身上  模板請設"群組"
	--戰鬥初始
	local _bossID = OwnerID();--_Guard
	local _roomID = ReadRoleValue( _bossID , EM_RoleValue_RoomID );
	DebugMsg( 0 , 0 , "--Lua_703176_AI_onFight ".._bossID.." RID : ".._roomID );

	if ( _Lua_703176_AI_GlobalVars == nil ) then Lua_703176_AI_reignite( _roomID ); return; end

	local _gConsts = _Lua_703176_AI_GlobalVars[ _roomID ];

	if ( _gConsts == nil ) then Lua_703176_AI_reignite( _roomID ); return; end
	...
	_Lua_703176_AI_GlobalVars[ _roomID ]._ctrlID = _ctrlID;
	_Lua_703176_AI_GlobalVars[ _roomID ].ExtObj = CreateDerivativeOnBoss;
	_Lua_703176_AI_GlobalVars[ _roomID ].DelExtObjs = _RemoveParasites;
	...
	--generate front door
	_Lua_703176_AI_GlobalVars[ _roomID ]._frontDoorID = Lua_703182_AddDoor( _gConsts._doorGUID , _gConsts._flagGUID , _gConsts._frontDoorFlagPos , _gConsts._roomID );

	BeginPlot( _ctrlID , "Lua_703176_AI_Engage" , 0 );--啟動戰鬥流程

	ScriptMessage(_bossID,0,2,"[Lua_703176_AI_OnFight]",0);

end--function Lua_703176_AI_onFight()

function Lua_703176_AI_Engage()
	local _roomID = ReadRoleValue( OwnerID() , EM_RoleValue_RoomID );--OwnerID在這裡指CTRLID
	DebugMsg( 0 , _roomID , "--Lua_703176_AI_Engage" );
	local _gConsts = _Lua_703176_AI_GlobalVars[ _roomID ];
	...
	while true do 
		...
		if ( _allBossAreDead == true ) then-- boss 全部死亡時 挑戰成功
			...
			Lua_703176_AI_Conquered( _roomID );
			break;
		end
		...
		if ( not _engaging ) then--都沒有存活玩家時 任務失敗
			...
			if ( #_rangePlayers <= 0 ) then
				...
				--Lua_703176_SweepAllRelativeBuff( _bossID , _gConsts._effectiveRange + 1000 , _gConsts._keyBuffs );--清除區域玩家所有相關BUFF
				Lua_703176_AI_Reset( _roomID );
				break;
			end
			]]--
			Lua_703176_AI_Reset( _roomID );
		...
		if ( _clock >= _frenzyCD and ( _isFrenzied == false ) ) then
				...
				ScriptMessage(_bossID,0,2,"[Lua_703176_AI_Frenzy]",0);
		end		
		...
		for i = 1 , 1 , 1 do--switch case simulation	--模擬 switch case 
			...
			for _bossName , _cdTime in pairs( _CD ) do--Boss重生	共存不息
				if ( _cdTime == 0 ) then
					_Lua_703176_AI_GlobalVars[ _roomID ]._bossVOs[_bossName]._id = Lua_703176_GenerateBoss( _bossVOs[_bossName] , _roomID );--重生對應的BOSS
					DebugMsg( 0 , _roomID , "--revive : ".._bossName );
					ScriptMessage( _bossID , 0 , 2 , tostring(_bossName).."[Lua_703176_AI_HasRevived]" , 0 );
				...
				if ( _cdTime > 0 and _cdTime == 10 ) then--重生倒數提示	每10秒
					ScriptMessage( _bossID , 0 , 2 , tostring(_bossName).."[Lua_703176_AI_WillReviveIn10sec]" , 0 );
				end
...
end--function Lua_703176_AI_Engage()

--++++++++++++++++++++++++++++++++++++++++++++++++++++++++++functions with shared values
function Lua_703176_AI_Reset( _roomID )
	DebugMsg( 0 , 0 , "--Lua_703176_AI_Reset" );
	...
	ScriptMessage(_gConsts._ctrlID,0,2,"[$MUTE]".."[Lua_703176_AI_Reset]",0);
	...
	--_Lua_703176_AI_GlobalVars[ _roomID ] = nil;	--clear all values
	...
	for _bossName , _bossVO in pairs( _bossVOs ) do--開啟戰鬥
		...
		_Lua_703176_AI_GlobalVars[ _roomID ]._bossVOs[_bossName]._id = Lua_703176_GenerateBoss( _bossVO , _roomID );--重生對應的BOSS
	end
	_Lua_703176_AI_GlobalVars[ _roomID ]._bossID = _gConsts._bossVOs._Guard._id;--presiding Boss --Guard
	--Lua_703176_BossIndividualsSetting( _gConsts );
	--SetDefIdleMotion( _gConsts._bossVOs._Schulize._id , ruFUSION_MIME_EMOTE_CRY );--讓Schulize哭

end--function Lua_703176_AI_Reset( _roomID )

function Lua_703176_AI_Conquered( _roomID )
	DebugMsg( 0 , 0 , "--Lua_703176_AI_Conquered" );
	local _gConsts = _Lua_703176_AI_GlobalVars[ _roomID ];--for read
	...
	ScriptMessage(_gConsts._ctrlID,0,2,"[$MUTE]".."[Lua_703176_AI_Conquered]",0);	
	...
	_gConsts.DelExtObjs();	--clear all extend items
	_Lua_703176_AI_GlobalVars[ _roomID ] = nil;	--clear all values
end--function Lua_703176_AI_Conquered( _roomID )
